...
public class SecurityConfig {
        ...
        /**
         * Spring just finds class that implements UserDetailsService
         * (UserRepositoryImpl in the case of this application).
         */
        ...
        public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
                ...
                http.exceptionHandling(exception -> exception.accessDeniedHandler(customAccessDeniedHandler) // Sets custom
                                                             // handler for
                                                             // Access Denied
                                                             // Error
                                                             .authenticationEntryPoint(customAuthenticationEntryPoint)); // Sets custom handler for non-logged user
                http.addFilterBefore(customAuthorizationFilter, UsernamePasswordAuthenticationFilter.class); // Adds filter that
                                                                                                            // checks user's
                                                                                                            // permissions for
                                                                                                            // every
                                                                                                            // HTTP-Request
                http.authorizeHttpRequests(request -> request.anyRequest().authenticated()); // All others requests must be
                                                                                            // authenticated
                return http.build();
        }
