...
public interface Table2Repository
                                  extends
                                  JpaRepository<Table2, Long>, //? Auto-creating of functions
                                  QuerydslPredicateExecutor<Table2>,
                                  Table2RepositoryCustomQueryDsl { //? Custom fucntions, created by hand
     ...
     */
    @EntityGraph(attributePaths = {"myOnlyOneTalbe1"})  //? Gets myOnlyOneTalbe1
    ...
    @Transactional
    @Lock (LockModeType.PESSIMISTIC_WRITE) //? If some session is working with some rows - blocks them and force another session to wait untill first session finishes its work or until some timeout has passed.
    Table2 saveAndFlush(Table2 givenTable1); //! It is also an update()
    ...
    Optional<Table2> customFunction(
